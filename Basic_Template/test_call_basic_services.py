...
if __name__ == '__main__':
    ...
    try:
        ...
        if opt == '1':
            ...
            jsonrpcclient.request(f"http://127.0.0.1:{jsonrpc_port}", jsonrpc_method, a=a, b=b)
        elif opt == '2':
            # Service TWO - Basic Echo
            jsonrpc_method = input('Which method (version|echo)? ')
            jsonrpc_port = registry['basic_service_two']['jsonrpc']
            jsonrpcclient.request(f"http://127.0.0.1:{jsonrpc_port}", jsonrpc_method, test="testing...")
